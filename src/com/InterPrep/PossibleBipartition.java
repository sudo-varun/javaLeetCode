...
public class PossibleBipartition {
    ...
    public boolean dfs(List<Integer>[] graph, Integer[] colors, int currNode, int currColor) {
        ...
        for(Integer adj: graph[currNode]) {
            if(colors[adj] == null) {
                if(!dfs(graph, colors, adj, currColor * -1)) return false;
            } else if(colors[adj] == currColor) return false;
        }
        return true;
    }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(possibleBipartition.possibleBipartition(10, dislikes));
    }
}
